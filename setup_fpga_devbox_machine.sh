...
# repo bin/ is the single source of truth (kept in sync with fpga_devbox.sh);
# editing a file there is enough to update the VM.
SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -n "$REPO_DIR" && -d "$REPO_DIR/bin" ]]; then
  REPO_BIN="$REPO_DIR/bin"
elif [[ -d "$SELF_DIR/bin" ]]; then
  REPO_BIN="$SELF_DIR/bin"
else
  REPO_BIN="${REPO_DIR:+$REPO_DIR/bin}"
fi

if [[ ! -d "$REPO_BIN" || ! -r "$REPO_BIN" ]]; then
  cat <<MSG >&2
Repo bin/ directory is not readable inside the VM: ${REPO_BIN:-<unset>}

The ~/bin helper scripts (fpga-launch-gui, start-vivado-gui, start-vitis-gui,
fpga-tool-shim, fpga-usb-setup, fpga-uart) now live in the fpga-devbox repo
bin/ directory and are copied into the VM during setup. Pass the repo directory
as the 2nd argument:

  $(basename "$0") "$INSTALLER_PATH" /absolute/path/to/fpga-devbox

The repo must live on a macOS path that OrbStack mounts into the VM (under
/Users, /Volumes, /private, ...) so it is readable at that same absolute path.
MSG
  exit 1
fi
